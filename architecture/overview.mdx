...
The registration happens in `src/lib/core/provide.ts` using `provide(TOKEN, factory)` calls, and `src/lib/index.ts` exposes the resolved instances via the `ioc` object:
...
// Also published to globalThis for convenience:
