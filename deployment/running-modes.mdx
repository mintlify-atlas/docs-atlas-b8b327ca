...
<Note>
  The web UI is available on port **60050** in all three modes. Pass the `--ui` flag when running locally, or set `UI=1` when running inside Docker. The UI provides a real-time dashboard showing signals, open positions, equity curves, and strategy logs — useful for monitoring paper and live runs as well as inspecting backtest results after the simulation completes.
</Note>
