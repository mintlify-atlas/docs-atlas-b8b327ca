...
<Note>
  This quickstart uses default credentials and a single-node setup suitable for local development. See the [Architecture Overview](/architecture/overview) page for a deep dive into atomicity guarantees and the Redis O(1) cache pattern, and the [Deployment](/deployment/docker-compose) page for production configuration including custom credentials, replica sets, and Redis Sentinel.
</Note>
